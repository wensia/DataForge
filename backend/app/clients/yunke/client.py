...
class YunkeApiClient(ABC):
    ...
    def __init__(
        ...
        auto_login_callback: Callable[[], Any] | None = None,
        max_retry: int = 1,
    ):
        """初始化客户端
        ...
        while retry_count <= self.max_retry:
            try:
                ...
                if retry_count > self.max_retry:
                    ...
                    logger.error(
                        f"登录重试次数已用尽: max_retry={self.max_retry}, error={error_msg}"
                    )
                ...
                if self.auto_login_callback:
                    ...
                    try:
                        ...
                        if result and result.get("success"):
                            # 更新cookies
                            new_cookies = result.get("cookies") or result.get(
                                "data", {}
                            ).get("cookies")
                            if new_cookies:
                                ...
                                logger.info(
                                    f"自动登录成功，已更新cookies: {list(new_cookies.keys())}"
                                )

                            # 更新user_id
                            new_user_id = result.get("user_id") or result.get(
                                "data", {}
                            ).get("id")
                            if new_user_id:
