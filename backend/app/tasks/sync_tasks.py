...
def _parse_datetime_to_timestamp(dt_str: str) -> int | None:
    """解析日期时间字符串为毫秒时间戳
...
def _convert_record_to_feishu(record: dict, field_mapping: list[dict]) -> dict:
    ...
    for mapping in field_mapping:
        ...
        if feishu_type == 5:  # 日期
            # 转换为毫秒时间戳
            timestamp = _parse_datetime_to_timestamp(str(value))
            if timestamp:
                value = timestamp
            else:
                continue  # 跳过无效日期
...
async def _run_sync_to_feishu(
    ...
    page_size: int,
    call_type: str,
    only_with_record: bool,
) -> dict:
    """执行云客通话记录同步到飞书（异步入口）

    Args:
        yunke_account_id: 云客账号ID
        feishu_client_id: 飞书客户端ID
    ...
    with Session(engine) as session:
        ...
        unique_field = next(
            (m for m in FEISHU_FIELD_MAPPING if m.get("is_unique")), FEISHU_FIELD_MAPPING[0]
        )
        ...
        feishu_records = [
            {"fields": _convert_record_to_feishu(r, FEISHU_FIELD_MAPPING)} for r in new_records
        ]
...
@celery_app.task(
    base=DataForgeTask,
    name="dataforge.sync_call_logs_to_feishu",
    bind=True,
    max_retries=3,
    default_retry_delay=120,
    lock_timeout=7200,  # 2小时超时
)
def sync_call_logs_to_feishu(
    self,
    yunke_account_id: int,
    feishu_client_id: int,
    bitable_app_token: str,
    ...
    return run_async(
        _run_sync_to_feishu(
            yunke_account_id=yunke_account_id,
            ...
            page_size=page_size,
            call_type=call_type,
            only_with_record=only_with_record,
        )
    )
